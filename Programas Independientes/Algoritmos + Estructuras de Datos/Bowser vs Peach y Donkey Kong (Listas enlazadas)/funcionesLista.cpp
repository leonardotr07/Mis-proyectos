...
struct Nodo * obtenerNodoAnterior(const struct Lista & tad, int elemento) {
    ...
    struct Nodo * recorrido = tad.cabeza;
    while ((recorrido != nullptr) and (recorrido->elemento.poderGuerrero < elemento)) {
        anterior = recorrido;
        recorrido = recorrido->siguiente;
    }
    return anterior;
...
int retornaCabeza(const struct Lista & lista){
    return lista.cabeza->elemento.poderGuerrero;
}
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            cout << recorrido->elemento.valorGuerrero;
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
    }
